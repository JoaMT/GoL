...
LedControl
lc2=LedControl(11,10,9
,dispositivos);//DATA | CLK | CS/LOAD | numbero de matrices

const unsigned char texto[] PROGMEM ={"       GAME OF LIFE           "};

long scrollDelay = 8;
unsigned long bufferLong [14] = {0}; 
unsigned long universe[16];
unsigned long universeNextFrame[16];
int frames=0;
int contadorVivas[2]={0,0};
byte turnoContador=0;
boolean turnoMarquesina=false;
...
void setup()
{
  ...
  if (random(2)>0) {
    ...
    for (int y=0;y<8;y++) {
      randomSeed(analogRead(0));
      buildGlider(random(32),random(16),random(4));
    }
    randomSeed(analogRead(0));
    buildLightSpaceship(random(32),random(16));
  }
...
void checkForRestart() {
  if (++frames>64) {
    ...
    if (contadorVivas[0] == contadorVivas[1]) {
      startGame();
    }
  }
}
...
void startGame() {
  ...
  if (turnoMarquesina) { turnoMarquesina=false; } else { turnoMarquesina=true; }
  scrollMessage(texto);
  randomSeed(analogRead(0));
